...
import { addToPlace, applyMovement } from "@/lib/stock-movements";
import {
  type ActionState,
  getRequiredString,
 ...
 *   `StockItem.quantity` goes through `applyMovement()` in `lib/stock-movements.ts`,
 *   which writes the row and its movement in one transaction. There is no second
 *   way to move stock.
