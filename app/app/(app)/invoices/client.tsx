...
export default function InvoicesClient({ locale, editionId, editionName, accounts, history, earningEntries, defaultTemplate }: Props) {
  ...
  const qrImageUrl = generatedInvoice
    ? `/api/qr/swiss?payload=${encodeURIComponent(generatedInvoice.payload)}`
    : null;
  ...
  async function handleGenerate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setGeneratedInvoice(nextGeneratedInvoice);
    } catch (err) {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      const response = await fetch("/api/documents/invoice/pdf", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          templateId: defaultTemplate.id,
          creditorName: generatedInvoice.creditorName,
          creditorAddress: generatedInvoice.creditorAddress,
          creditorPostalCode: generatedInvoice.creditorPostalCode,
          creditorCity: generatedInvoice.creditorCity,
          creditorCountry: generatedInvoice.creditorCountry,
          bankAccountName: generatedInvoice.bankAccountName,
          iban: generatedInvoice.iban,
          supplierName: generatedInvoice.supplierName,
          supplierAddress: generatedInvoice.supplierAddress,
          supplierPostalCode: generatedInvoice.supplierPostalCode,
          supplierCity: generatedInvoice.supplierCity,
          supplierCountry: "CH",
          invoiceNumber: generatedInvoice.invoiceNumber,
          header: generatedInvoice.header,
          invoiceDate: generatedInvoice.invoiceDate,
          dueDate: generatedInvoice.dueDate,
          paymentReference: generatedInvoice.reference,
          message: generatedInvoice.message,
          totalAmount: generatedInvoice.totalAmount,
          lineItems: generatedInvoice.lineItems,
          qrPayload: generatedInvoice.payload,
        }),
      });
      ...
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = `${generatedInvoice.invoiceNumber || "invoice"}.pdf`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <h1 className="text-3xl font-semibold tracking-tight">{copy.invoices.title} {editionName}</h1>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_420px]">
        <section className="rounded-[28px] border border-[var(--line)] bg-[var(--panel-strong)] p-6">
          <h2 className="text-xl font-semibold">{copy.invoices.create}</h2>
          ...
          <div className="mt-8 border-t border-[var(--line)] pt-6">
            <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">{copy.invoices.historyTitle}</h3>
            {invoiceHistory.length === 0 ? (
              ...
              <div className="mt-3 space-y-2">
                {invoiceHistory.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-xl border border-[var(--line)] bg-[var(--panel)] px-4 py-3 text-left text-sm"
                  >
                    <div className="flex w-full items-center justify-between gap-3">
                      <button
                        type="button"
                        onClick={() => refillFromHistory(item)}
                        className="flex min-w-0 flex-1 items-center justify-between text-left hover:text-[var(--accent)]"
                      >
                        <div>
                          <p className="font-semibold">{item.invoiceNumber}</p>
                          <p className="text-xs text-[var(--muted)]">{item.supplierName} • {item.invoiceDate}</p>
                          <p className="text-xs text-[var(--muted)]">
                            {item.paidAt
                              ? `${copy.invoices.statusPaid}${item.linkedJournalEntryLabel ? ` • ${item.linkedJournalEntryLabel}` : ""}`
                              : copy.invoices.statusUnpaid}
                          </p>
                        </div>
                        <p className="text-xs font-semibold">{formatCurrency(item.totalAmount)}</p>
                      </button>
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="inline-flex rounded-full border border-[var(--line)] px-3 py-1.5 text-xs font-semibold hover:bg-[var(--panel-strong)] disabled:opacity-60"
                        >
        ...
        <section className="rounded-[28px] border border-[var(--line)] bg-[var(--panel-strong)] p-6">
          <h2 className="text-xl font-semibold">{copy.invoices.preview}</h2>

          {!generatedInvoice ? (
            <p className="mt-4 text-sm text-[var(--muted)]">{copy.invoices.qrLabel}</p>
          ) : (
            <div className="mt-5 space-y-4 text-sm">
              <div className="space-y-1">
                <p><span className="text-[var(--muted)]">{copy.invoices.bankAccount}:</span> {generatedInvoice.bankAccountName}</p>
                {generatedInvoice.invoiceNumber && <p><span className="text-[var(--muted)]">{copy.invoices.invoiceNumber}:</span> {generatedInvoice.invoiceNumber}</p>}
                <p><span className="text-[var(--muted)]">{copy.invoices.invoiceDate}:</span> {generatedInvoice.invoiceDate}</p>
                {generatedInvoice.dueDate && <p><span className="text-[var(--muted)]">{copy.invoices.dueDate}:</span> {generatedInvoice.dueDate}</p>}
                {generatedInvoice.header && <p><span className="text-[var(--muted)]">{copy.invoices.header}:</span> {generatedInvoice.header}</p>}
              </div>

              <div className="border-t border-[var(--line)] pt-3 space-y-1">
                <p>
                  <span className="text-[var(--muted)]">{copy.invoices.supplierName}:</span>{" "}
                  <span className="whitespace-pre-line">{generatedInvoice.supplierName}</span>
                </p>
                <p><span className="text-[var(--muted)]">{copy.invoices.supplierAddress}:</span> {generatedInvoice.supplierAddress}, {generatedInvoice.supplierPostalCode} {generatedInvoice.supplierCity}</p>
              </div>

              {generatedInvoice.lineItems.length > 0 && (
                <div className="border-t border-[var(--line)] pt-3">
                  <table className="w-full text-xs">
                    <thead className="text-[var(--muted)]">
                      <tr>
                        <th className="text-left font-medium">{copy.invoices.description}</th>
                        <th className="text-right font-medium">{copy.invoices.quantity}</th>
                        <th className="text-right font-medium">{copy.invoices.unitPrice}</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <tr className="border-t border-[var(--line)] font-semibold">
                        <td colSpan={2} className="text-right">{copy.invoices.total}:</td>
                        <td className="text-right">{formatCurrency(generatedInvoice.lineItems.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0))}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              {qrImageUrl ? (
                <>
                  <Image
                    src={qrImageUrl}
                    alt={copy.invoices.qrLabel}
                    width={320}
                    height={320}
                    unoptimized
                    className="w-full max-w-[280px] rounded-xl border border-[var(--line)] bg-white p-2"
                  />
                  <a
                    href={qrImageUrl}
                    download="swiss-qr-invoice.svg"
                    className="inline-flex rounded-full border border-[var(--line)] px-4 py-2 text-xs font-semibold hover:bg-[var(--panel)]"
                  >
                    {copy.invoices.downloadQr}
                  </a>
                  <button
                    type="button"
                    onClick={handleDownloadPdf}
                    disabled={downloadingPdf}
                    className="inline-flex rounded-full bg-[var(--accent)] px-4 py-2 text-xs font-semibold text-white hover:bg-[var(--accent-strong)] disabled:opacity-60"
                  >
                    {downloadingPdf ? "..." : copy.invoices.generatePdf}
                  </button>
                </>
              ) : null}
            </div>
