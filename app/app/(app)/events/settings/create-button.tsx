...
const FORM_ID = "create-event-type-form";

/** The one thing this screen creates, the way everything else in the app creates. */
export function EventSettingsCreateButton({ locale }: { locale: Locale }) {
  const copy = dictionaries[locale].events;
  const shellCopy = dictionaries[locale].shell;

  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState(createEventTypeAction, initialActionState);
